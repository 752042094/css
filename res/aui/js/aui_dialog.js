    ...
    auiDialog.prototype = {
        ...
        create: function(params, callback) {
            ...
            if (params.input) {
                ...
                var aui_dialog_input_id = 'aui_dialog_input_' + new Date().getTime();
                var msgHtml = '<div class="aui-dialog-body"><input id="' + aui_dialog_input_id + '" type="' + params.type + '" placeholder="' + params.text + '"></div>';
            } else {
                        ...
                        if (callback) {
                            if (params.input) {
                                callback({
                                    ...
                                    text: document.querySelector("input#"+aui_dialog_input_id).value
                                });
                            } else {
                                callback({
                                    buttonIndex: parseInt(this.getAttribute("button-index")) + 1
        ...
        open: function(params) {
            ...
            document.querySelector(".aui-mask").addEventListener("touchstart", function(e) {
                e.preventDefault(); //防止点到遮罩后面元素
                var maskclick = params.maskclick ? params.maskclick : self.params.maskclick;
                if(maskclick){
                  self.close();
                  return;
                }
